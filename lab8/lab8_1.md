...
(1) Each variable does not have it's own column
(2) each observation does not have its own row 
(3) each value has its own cell. 
...
Each variable does not have its own column. each observation does not have its own row!
...
plant_data %>%
  pivot_longer(
   ...
   values_to = "progress",
   values_drop_na = TRUE) %>%
  select(genotype, day, progress)
```

```
## # A tibble: 90 x 3
##    genotype day   progress
##    <chr>    <chr>    <dbl>
##  1 control  1         21.7
##  2 control  2         19.9
##  3 control  3         20.7
