...
```
## here() starts at D:/TA files/Winter2021 BIS15L/students_rep/BIS15W2021_jviswanathan
```
...
Yes, they seem tidy. each value has its own row, and each variable is its own column. also, each entry fits into 1 cell. 
</div>
...
```r
enterococci_average <- sydneybeaches_long %>%
  separate(date, into = c("day", "month", "year")) %>%
  group_by(site, year) %>%
  summarize(average_entero = mean(enterococci_cfu_100ml, na.rm = T))
```
...
```r
enterococci_average_wide <- enterococci_average %>%
  pivot_wider(
    names_from = "site",
    values_from = "average_entero"
  )
